window.historyTexts =
  window.historyTexts || {};

window.historyTexts.tr =
  window.historyTexts.tr || {};

window.historyTexts.tr.baktun =
  window.historyTexts.tr.baktun || {};

window.historyTexts.tr.baktun.actualiseren_oorsprong = {

  title:
    "Gerçekleştirmek",

text:
  `Gerçekleştirmek + Kaynak

<br><br>

MÖ 3100 civarında Mezopotamya'da yazı, giderek daha karmaşık bilgilerin kil tabletler üzerine kaydedilmesini sağlayan pratik bir sisteme dönüşüyordu ve bu, tam gelişmiş çivi yazısına doğru önemli bir adım oluşturdu.`

};

window.historyTexts.tr.baktun.onderscheiden_stroming = {

  title:
    "Kabul Etmek",

text:
  `Ayırt Etmek + Akış

<br><br>

MÖ 2560 civarında Gize'deki Büyük Piramit, Firavun Khufu'nun hükümdarlığı sırasında tamamlandı. İnşası, muazzam miktarda taşın koordineli biçimde taşınmasını gerektirdi ve antik Mısır mühendisliğinin en dikkat çekici başarılarından biri olmaya devam ediyor.`

};

window.historyTexts.tr.baktun.transformeren_interactie = {

  title:
    "Ayırt Etmek",

text:
  `Dönüştürmek + Etkileşim

<br><br>

MÖ 2200 civarında Mohenjo-daro ve Harappa gibi İndus Vadisi Uygarlığı şehirleri; planlı sokaklar, standartlaştırılmış tuğla yapılar, drenaj ağları ve topluluklar arasında kapsamlı alışverişi içeren son derece düzenli kentsel sistemler geliştirmişti.`

};

window.historyTexts.tr.baktun.selecteren_relatie = {

  title:
    "Konumlandırmak",

text:
  `Seçmek + İlişki

<br><br>

MÖ 1750 civarında Hammurabi Kanunları Babil'de yazıya geçirildi ve ticaret, mülkiyet, aile, emek ve toplum içindeki diğer ilişkilerle ilgili yasaları ve hukuki kararları bir araya getirdi.`

};

window.historyTexts.tr.baktun.afstemmen_waarheid = {

  title:
    "Seçmek",

text:
  `Uyumlamak + Hakikat

<br><br>

MÖ 1200 civarında Doğu Akdeniz ve Yakın Doğu'daki birçok büyük toplum Geç Tunç Çağı çöküşü sırasında geriledi veya çöktü. Saray sistemleri ortadan kalktı, ticaret ağları kesintiye uğradı ve birçok şehir terk edildi veya yıkıldı.`

};

window.historyTexts.tr.baktun.ontvouwen_verbeelding = {

  title:
    "İyileştirmek",

text:
  `Açmak + Hayal Gücü

<br><br>

MÖ 1100 civarında Fenike alfabesi, ünsüz sesleri temsil eden az sayıda işarete dayalı kompakt bir yazı sistemi olarak kullanılıyordu. Akdeniz'deki ticaret ağları aracılığıyla yayılması, Yunan alfabesi de dahil olmak üzere daha sonraki alfabetik geleneklerin şekillenmesine katkıda bulundu.`

};

window.historyTexts.tr.baktun.onderscheiden_structuur_athene = {

  title:
    "Uyumlamak",

text:
  `Ayırt Etmek + Yapı

<br><br>

MÖ 508 civarında Kleisthenes ile ilişkilendirilen reformlar Atina'nın siyasi yapısını yeniden düzenledi ve daha geniş bir erkek yurttaş kitlesinin kamusal karar alma süreçlerine doğrudan katılabildiği bir sistemin kurulmasına yardımcı oldu.`

};

window.historyTexts.tr.baktun.actualiseren_manifestatie_rome = {

  title:
    "Dönüştürmek",

text:
  `Gerçekleştirmek + Tezahür

<br><br>

MÖ 27 civarında Octavianus'a Augustus unvanı verildi; bu, Roma Principatus döneminin başlangıcını ve Roma Cumhuriyeti'nin yeni bir imparatorluk siyasi düzenine dönüşümünü simgeledi.`

};

window.historyTexts.tr.baktun.selecteren_bewustzijn = {

  title:
    "Bütünleştirmek",

text:
  `Seçmek + Bilinç

<br><br>

MS 313 yılında Konstantin ve Licinius Milano Fermanı'nı yayımlayarak Roma İmparatorluğu'nda dini hoşgörüyü tesis etti ve Hristiyanların dinlerini açıkça uygulamalarına ve daha önce el konulan mülklerini geri almalarına izin verdi.`

};

window.historyTexts.tr.baktun.transformeren_relatie = {

  title:
    "Kavramak",

text:
  `Dönüştürmek + İlişki

<br><br>

MS 622 yılında Muhammed ve takipçileri Hicret olarak bilinen olayla Mekke'den Medine'ye göç etti. Bu göç, erken dönem Müslüman topluluğunun oluşumunda belirleyici bir aşamaya işaret etti ve daha sonra İslami takvimin başlangıç noktası oldu.`

};

window.historyTexts.tr.baktun.onderscheiden_expressie = {

  title:
    "Kalıcılaştırmak",

text:
  `Ayırt Etmek + İfade

<br><br>

MS 1040 civarında Bi Sheng, Çin'de pişmiş kilden yapılmış ayrı ve yeniden kullanılabilir karakterler kullanarak hareketli harf baskısını geliştirdi. Bu yöntem, karakterlerin farklı kombinasyonlarda düzenlenmesine, basılmasına ve yeniden kullanılmasına olanak sağladı.`
};

window.historyTexts.tr.baktun.vrijmaken_bewustzijn = {

  title:
    "Açmak",

text:
  `Serbest Bırakmak + Bilinç

<br><br>

MS 1492 yılında Kristof Kolomb, İspanyol Kraliyeti tarafından finanse edilen bir yolculuk sırasında Amerika'ya ulaştı. Bu yolculuk Avrupa ile Amerika arasında kalıcı temasın başlamasına ve insanların, bitkilerin, hayvanların, hastalıkların ve malların Atlantik boyunca taşındığı Kolomb Takası'na yol açtı.`

};

window.historyTexts.tr.baktun.verankeren_waarheid = {

  title:
    "Serbest Bırakmak",

text:
  `Kalıcılaştırmak + Hakikat

<br><br>

MS 1969 yılında ARPANET üzerinden ilk mesaj UCLA ile Stanford Araştırma Enstitüsü'ndeki bilgisayarlar arasında iletildi. Bu erken bilgisayar ağı, modern internetin önemli bir öncüsü oldu ve dijital bilginin birbirinden uzaktaki bilgisayar sistemleri arasında nasıl aktarılabileceğini gösterdi.`

};

window.historyTexts.tr.baktun.erkennen_informatie = {

  title:
    "Gerçekleştirmek",

text:
  `Kabul Etmek + Bilgi

<br><br>

MÖ 3100 civarında Yukarı ve Aşağı Mısır, kademeli bir siyasi birleşme süreci içinde tek bir krallık altında bir araya getirildi. Narmer, bu geçişle ilişkilendirilen en erken hükümdarlardan biridir ve gelişmekte olan Mısır devletinin kalıcı bir simgesi hâline gelmiştir.

<br><br>

Birleşme, Nil boyunca daha merkezi bir siyasi yapı oluşturdu ve binlerce yıl boyunca Antik Mısır uygarlığını şekillendirecek hanedan sisteminin temellerinin atılmasına yardımcı oldu.`

};

window.historyTexts.tr.baktun.actualiseren_essentie = {

  title:
    "Kabul Etmek",

text:
  `Gerçekleştirmek + Öz

<br><br>

MÖ 2350 civarında Akkadlı Sargon, çok sayıda şehir devletini ve bölgeyi Akkad yönetimi altında bir araya getirerek Mezopotamya'da yeni bir siyasi düzen kurdu. Böylece tarihte bilinen en eski büyük bölgesel imparatorluklardan biri ortaya çıktı.

<br><br>

Akkad devleti, daha önce ayrı olan siyasi merkezleri ortak bir yönetim sistemi, askerî güç ve kraliyet otoritesi aracılığıyla birbirine bağlayarak Mezopotamya'nın siyasi yapısını yeniden şekillendirdi.`

};

window.historyTexts.tr.baktun.vrijmaken_levenspad = {

  title:
    "Ayırt Etmek",

text:
  `Serbest Bırakmak + Yaşam Yolu

<br><br>

MÖ 2100 civarında, Mezopotamya'daki Üçüncü Ur Hanedanı döneminde, Kral Ur-Nammu veya halefi Şulgi yönetiminde yazılı bir yasa derlemesi yayımlandı. Günümüze ulaşan parçalar; mülkiyet, evlilik, bedensel yaralanma ve hukuki anlaşmazlıklar gibi konulara ilişkin kurallar içermektedir.

<br><br>

Ur-Nammu Kanunları, günümüzde bilinen büyük ölçüde korunmuş en eski yasa derlemesidir. Daha iyi bilinen Hammurabi Kanunları'ndan yüzyıllar önce, sosyal ilişkileri düzenlemek ve anlaşmazlıkları çözmek için yazılı hukuk sistemlerinin kullanıldığını göstermektedir.`

};

window.historyTexts.tr.baktun.erkennen_interactie = {

  title:
    "Konumlandırmak",

text:
  `Kabul Etmek + Etkileşim

<br><br>

MÖ 1800 civarında Mısır'da ve Sina Yarımadası'nda erken alfabetik yazı ortaya çıktı. İşaretleri kısmen Mısır hiyerogliflerinin biçimlerinden türetilmişti, ancak çok daha küçük bir konuşma sesi kümesini temsil etmek için kullanılıyordu.

<br><br>

Bu erken yazıtlar, daha sonra Kenan ve Fenike yazı sistemleriyle devam eden alfabetik bir geleneğin gelişiminin parçasıdır. Sınırlı sayıda işaretin kullanılması, yüzlerce işaret gerektiren yazı sistemlerine kıyasla dili kaydetmek için temelden farklı bir yaklaşım sağladı.`

};

window.historyTexts.tr.baktun.onderscheiden_bewustzijn = {

  title:
    "Seçmek",

text:
  `Ayırt Etmek + Bilinç

<br><br>

MÖ 1274 civarında Mısır Firavunu II. Ramses ile Hitit Kralı II. Muvatalli'nin orduları, günümüz Suriye'sindeki Kadeş kenti yakınlarında savaştı. Bu savaş, ayrıntılı biçimde belgelenmiş en eski askerî çatışmalardan biridir ve Mısır yazıtları ile anıtsal kabartmalarda kayıt altına alınmıştır.

<br><br>

İki imparatorluk da kalıcı ve kesin bir zafer elde edemedi. Yaklaşık on beş yıl sonra Mısır ile Hitit İmparatorluğu, diplomatik ilişkiler ve karşılıklı yükümlülükler belirleyen resmî bir barış antlaşması imzaladı ve böylece günümüze ulaşan en eski uluslararası barış antlaşması örneklerinden biri ortaya çıktı.`

};

window.historyTexts.tr.baktun.erkennen_samenhang = {

  title:
    "İyileştirmek",

text:
  `Kabul Etmek + Bütünlük

<br><br>

MÖ 776 yılında Yunan geleneği, Olympia'daki ilk Olimpiyat galibini kaydeder. Burada atletik yarışmalar, Zeus onuruna düzenlenen dinsel bir festivalin parçası olarak gerçekleştiriliyor ve farklı Yunan topluluklarından katılımcıları ve seyircileri kutsal alanda bir araya getiriyordu.

<br><br>

Olimpiyat festivali yüzyıllar boyunca devam etti ve daha sonraki Yunan tarihçileri, dört yıllık Olimpiyat dönemlerini tarihsel olayları zaman içinde düzenlemek için bir sistem olarak kullandı. MÖ 776 geleneksel tarihi, Olympia'da gerçekleştirilen ilk atletik yarışmanın tarihini değil, günümüze ulaşan Olimpiyat galipleri listelerinin başlangıcını ifade eder.`

};

window.historyTexts.tr.baktun.transformeren_waarheid = {

  title:
    "Uyumlamak",

text:
  `Dönüştürmek + Hakikat

<br><br>

MÖ 399 yılında Atinalı filozof Sokrates, dinsizlik ve gençleri yozlaştırmakla suçlanarak yargılandı, suçlu bulundu ve ölüm cezasına çarptırıldı. Sokrates kendi yazılı eserlerini bırakmadı; onun hakkında bilinenlerin büyük bölümü öğrencilerinin ve çağdaşlarının, özellikle Platon ve Ksenophon'un aktardıklarından gelmektedir.

<br><br>

Israrlı biçimde sorular sormaya dayanan yöntemi, varsayımları, tanımları ve bilgi iddialarını diyalog yoluyla inceleyen Batı felsefesinin en etkili geleneklerinden biri hâline geldi.`

};

window.historyTexts.tr.baktun.onderscheiden_grenzen = {

  title:
    "Dönüştürmek",

text:
  `Ayırt Etmek + Sınırlar

<br><br>

MÖ üçüncü yüzyılda Sirakuzalı Arşimet, geometriyi, mekaniği ve cisimlerin sıvılar içindeki davranışını incelemek için matematiksel yöntemler geliştirdi. Günümüze ulaşan çalışmaları kaldıraçlar, ağırlık merkezleri, kaldırma kuvveti, alanlar ve hacimler üzerine incelemeler içermektedir.

<br><br>

Arşimet, matematiksel akıl yürütmeyi fiziksel problemlere uygulayarak doğa olaylarının nicel ilişkiler aracılığıyla nasıl tanımlanabileceğini gösterdi. Çalışmaları matematiğin, mekaniğin ve matematiksel fiziğin daha sonraki gelişimi için önemli bir temel oluşturdu.`

};

window.historyTexts.tr.baktun.transformeren_potentieel = {

  title:
    "Bütünleştirmek",

text:
  `Dönüştürmek + Potansiyel

<br><br>

MS ikinci yüzyılın başlarında Han Çin'inde kâğıt yapımı daha da geliştirildi. Çin tarihî kaynakları, devlet görevlisi Cai Lun'u üretim sürecindeki iyileştirmelerle ve MS 105 yılında bitkisel lifler ile diğer malzemelerden kâğıt yapma yöntemini imparatorluk sarayına sunmasıyla ilişkilendirir.

<br><br>

Arkeolojik kanıtlar, Cai Lun'dan önce de Çin'de çeşitli kâğıt biçimlerinin bulunduğunu göstermektedir. Kâğıt yapımının giderek geliştirilmesi, zamanla bilginin kaydedilmesini, korunmasını ve aktarılmasını kökten değiştirecek hafif ve çok yönlü bir yazı malzemesi ortaya çıkardı.`

};

window.historyTexts.tr.baktun.integreren_expressie = {

  title:
    "Kavramak",

text:
  `Bütünleştirmek + İfade

<br><br>

MS 628 yılında Hintli matematikçi Brahmagupta, sıfır ve negatif sayılarla hesaplama yapmak için açık kurallar formüle ettiği matematik ve astronomi eseri Brāhmasphuṭasiddhānta'yı tamamladı.

<br><br>

Sıfırın Hint matematiğinde daha uzun bir geçmişi vardı, ancak Brahmagupta'nın sistematik yaklaşımı onun aritmetik içinde bir sayı olarak gelişmesinde önemli bir adım oluşturdu. Çalışmaları ayrıca cebir, geometri ve astronomik hesaplama yöntemlerinin gelişimine katkıda bulundu.`

};

window.historyTexts.tr.baktun.erkennen_relatie = {

  title:
    "Kalıcılaştırmak",

text:
  `Kabul Etmek + İlişki

<br><br>

On birinci yüzyılın başlarında İbnü'l-Heysem, etkili eseri Optik Kitabı'nda ışık ve görme üzerine sistematik bir inceleme geliştirdi. Görmenin, nesnelerden yansıyan ışığın gözlere girmesiyle gerçekleştiğini açıklayarak görsel ışınların gözlerin kendisinden yayıldığını öne süren teorileri reddetti.

<br><br>

Araştırmaları geometriyi; ışık, yansıma ve kırılma üzerine gözlem ve deneylerle birleştirdi. Bu çalışma, optiğin gelişimine ve doğa olaylarının incelenmesinde deneysel araştırmanın kullanılmasına önemli katkılarda bulundu.`

};

window.historyTexts.tr.baktun.onderscheiden_manifestatie = {

  title:
    "Açmak",

text:
  `Ayırt Etmek + Tezahür

<br><br>

1543 yılında Nicolaus Copernicus, Dünya'nın kendi ekseni etrafında döndüğü ve diğer gezegenlerle birlikte Güneş'in çevresinde dolandığı matematiksel bir model sunduğu De revolutionibus orbium coelestium adlı eserini yayımladı.

<br><br>

Güneş merkezli fikirler Antik Çağ'da da mevcut olsa da Copernicus bunları ayrıntılı bir astronomik sistem hâline getirdi. Çalışması, uzun süredir yerleşmiş olan Dünya merkezli modele meydan okudu ve Kepler, Galileo ve diğerlerinin daha sonraki astronomik çalışmalarına önemli bir temel oluşturdu.`

};

window.historyTexts.tr.baktun.verfijnen_structuur = {

  title:
    "Serbest Bırakmak",

text:
  `İnceltmek + Yapı

<br><br>

On dokuzuncu yüzyılda mucitler, metinleri mekanik olarak üretmek için giderek daha kullanışlı makineler geliştirdi. 1868 yılında Christopher Latham Sholes, Carlos Glidden ve Samuel W. Soulé, modern daktilonun gelişimine katkıda bulunan bir yazı makinesi için patent aldı.

<br><br>

1874'ten itibaren Remington tarafından ticari bir modeli üretilmeye başlandı. Klavye düzeni QWERTY diziliminin yerleşmesine katkıda bulunurken, daktiloların giderek yaygınlaşması mektupların, belgelerin ve idari kayıtların hazırlanma biçimini önemli ölçüde değiştirdi.`

};

window.historyTexts.tr.baktun.afstemmen_oorsprong = {

  title:
    "Serbest Bırakmak",

text:
  `Uyumlamak + Kaynak

<br><br>

1628 yılında İngiliz hekim William Harvey, De Motu Cordis adlı eserini yayımlayarak kanın vücutta sürekli dolaştığını ve kalp tarafından pompalandığını gösteren deneysel ve nicel kanıtlar sundu.

<br><br>

Harvey, kalbin kanı atardamarlar boyunca hareket ettirdiğini ve kanın toplardamarlar aracılığıyla kalbe geri döndüğünü gösterdi. Çalışması, kanın hareketine ilişkin daha eski açıklamaları geçersiz kıldı ve kardiyovasküler sistemin modern biçimde anlaşılmasının önemli temellerinden biri oldu.`

};

window.historyTexts.tr.baktun.transformeren_informatie = {

  title:
    "Serbest Bırakmak",

text:
  `Dönüştürmek + Bilgi

<br><br>

1642 yılında Fransız matematikçi Blaise Pascal, babasının işiyle ilgili idari hesaplamalara yardımcı olmak amacıyla mekanik bir hesap makinesi geliştirmeye başladı. Daha sonra Pascaline olarak bilinen bu makine, aritmetik işlemleri mekanik olarak gerçekleştirmek için birbirine bağlı çarklar ve dişliler kullanıyordu.

<br><br>

Pascal tasarımı geliştirmeye devam etti ve makinenin çeşitli sürümlerini üretti. Pascaline, sayısal hesaplamaların zihinsel veya yazılı bir işlem olmaktan çıkarılıp mekanik bir aygıtın işleyişine aktarılmasının önemli erken örneklerinden biri oldu.`

};

window.historyTexts.tr.baktun.integreren_verbeelding = {

  title:
    "Serbest Bırakmak",

text:
  `Bütünleştirmek + Hayal Gücü

<br><br>

1665 yılında Robert Hooke, mikroskopla yaptığı ayrıntılı gözlemleri, normalde çıplak gözle görülemeyen yapıların çarpıcı çizimleriyle birlikte sunduğu Micrographia adlı eserini yayımladı.

<br><br>

Hooke, ince mantar kesitlerini incelerken gözlemlediği küçük bölmeleri "hücreler" olarak tanımladı. Çalışması, daha önce gizli kalan mikroskobik bir dünyanın görünür hâle gelmesine yardımcı oldu ve insan algısını genişletmek için araçların, gözlemin ve görsel temsilin nasıl bir araya getirilebileceğini gösterdi.`

};

window.historyTexts.tr.baktun.doorgronden_potentieel = {

  title:
    "Serbest Bırakmak",

text:
  `Kavramak + Potansiyel

<br><br>

1687 yılında Isaac Newton, hareket yasalarını formüle ettiği ve evrensel kütleçekimin matematiksel bir açıklamasını geliştirdiği Philosophiae Naturalis Principia Mathematica adlı eserini yayımladı.

<br><br>

Newton, Dünya üzerindeki nesnelerin hareketi ile gök cisimlerinin hareketinin aynı fiziksel ilkeler aracılığıyla anlaşılabileceğini gösterdi. Çalışması, hareketi öngörmek için güçlü bir matematiksel çerçeve sağladı ve klasik mekaniğin ve fiziğin önemli temellerinden biri oldu.`

};

window.historyTexts.tr.baktun.verankeren_energie = {

  title:
    "Serbest Bırakmak",

text:
  `Kalıcılaştırmak + Enerji

<br><br>

1712 yılında Thomas Newcomen, madenlerden su pompalamak için kullanılabilecek pratik bir atmosferik buhar makinesi geliştirdi. Buhar, silindirin içinde yoğunlaştırılarak kısmi bir vakum oluşturuyor ve atmosfer basıncının pistonu hareket ettirmesini sağlıyordu.

<br><br>

Makine, ısıyı pratik ölçekte mekanik işe dönüştürmenin yeni bir yolunu sağladı. Newcomen makineleri madencilik bölgelerinde yaygınlaştı ve Sanayi Devrimi sırasında buhar gücünün daha sonraki gelişimi için önemli bir temel oluşturdu.`

};

window.historyTexts.tr.baktun.ontsluiten_grenzen = {

  title:
    "Serbest Bırakmak",

text:
  `Açmak + Sınırlar

<br><br>

1728 yılında İngiliz astronom James Bradley, günümüzde yıldız ışığının aberasyonu olarak bilinen olguyu açıkladı. Hassas gözlemler, yıldızların görünen konumlarının yıl boyunca küçük miktarlarda değiştiğini gösterdi.

<br><br>

Bradley bu etkiyi, Dünya'nın Güneş çevresindeki hareketi ile ışığın sonlu hızının birleşimiyle açıkladı. Keşfi, Dünya'nın yörüngesel hareketine ilişkin önemli gözlemsel kanıtlar sağladı ve gözlemcinin kendi hareketinin uzak nesnelerin görünen konumunu nasıl etkileyebileceğini ortaya koydu.`

};

window.historyTexts.tr.baktun.vrijmaken_interactie = {

  title:
    "Serbest Bırakmak",

text:
  `Serbest Bırakmak + Etkileşim

<br><br>

1752 yılında Benjamin Franklin ile ilişkilendirilen deneyler, yıldırım ile laboratuvar ortamında incelenen elektriğin aynı fiziksel olgunun farklı görünümleri olduğunu göstermeye yardımcı oldu. Franklin, fırtına bulutlarının elektriksel doğasını sınamak için yöntemler önermiş ve bununla ilişkili deneyler Avrupa ve Kuzey Amerika'da başarıyla gerçekleştirilmişti.

<br><br>

Bu anlayış aynı zamanda elektrik yükünü güvenli bir şekilde toprağa ileterek yıldırım çarpmalarının yol açabileceği hasar riskini azaltmanın pratik bir yolunu sağlayan paratonerin geliştirilmesine de yol açtı.`

};

window.historyTexts.tr.baktun.actualiseren_waarde = {

  title:
    "Serbest Bırakmak",

text:
  `Gerçekleştirmek + Değer

<br><br>

1776 yılında İskoç ekonomist ve filozof Adam Smith, An Inquiry into the Nature and Causes of the Wealth of Nations adlı eserini yayımladı. Eser, emek, uzmanlaşma, üretim ve mübadelenin ekonomik zenginliğin oluşmasına ve dağılımına nasıl katkıda bulunduğunu inceledi.

<br><br>

Smith, iş bölümünü, piyasaları, fiyatları ve bireysel ekonomik faaliyet ile daha geniş toplum arasındaki ilişkiyi analiz etti. Kitap, modern ekonominin temel eserlerinden biri hâline geldi ve malların, emeğin ve değerin ekonomiler içinde nasıl düzenlendiğinin incelenmesi üzerinde kalıcı bir etki bıraktı.`

};

window.historyTexts.tr.baktun.erkennen_stroming = {

  title:
    "Serbest Bırakmak",

text:
  `Kabul Etmek + Akış

<br><br>

1785 yılında İskoç jeolog James Hutton, jeolojik değişimin çok uzun zaman dilimleri boyunca sürekli işleyen süreçlerden kaynaklandığı bir Dünya teorisi sundu. Yeryüzünü tekrar tekrar şekillendiren erozyon, tortulaşma, kayaç oluşumu ve yükselme döngülerini tanımladı.

<br><br>

Hutton'ın yorumu, yeryüzü şekillerinin ve kaya oluşumlarının kayıtlı insanlık tarihinden çok daha eski bir geçmişin kanıtlarını taşıdığını gösterdi. Çalışması, modern jeolojinin ve derin jeolojik zaman kavramının önemli temellerinden biri oldu.`

};

window.historyTexts.tr.baktun.onderscheiden_relatie = {

  title:
    "Serbest Bırakmak",

text:
  `Ayırt Etmek + İlişki

<br><br>

On dokuzuncu yüzyılın başlarında İngiliz bilim insanı John Dalton, her kimyasal elementin kendine özgü bir atom türünden oluştuğunu öne süren bir atom teorisi geliştirdi. Farklı elementlerin atomlarının belirli oranlarda birleşmesiyle kimyasal bileşiklerin oluştuğunu ileri sürdü.

<br><br>

Dalton, kimyasal tepkimelerde gözlemlenen düzenleri açıklamak için atom ağırlıklarını ve basit sayısal oranları kullandı. Teorisi, maddenin bileşimini anlamak için yeni bir çerçeve sağladı ve modern kimyanın gelişiminin önemli temellerinden biri oldu.`

};

window.historyTexts.tr.baktun.plaatsen_expressie = {

  title:
    "Serbest Bırakmak",

text:
  `Konumlandırmak + İfade

<br><br>

Joseph Nicéphore Niépce, yaklaşık 1826 veya 1827 yılında View from the Window at Le Gras adlı görüntüyü oluşturdu; bu, bir kamerayla üretildiği bilinen günümüze ulaşmış en eski fotoğraftır. Üst kattaki bir pencereden görünen manzarayı kaydetmek için camera obscura ve metal bir plaka üzerindeki ışığa duyarlı bir kaplama kullandı.

<br><br>

Pozlama, sahneden gelen ışığın mekânsal düzenini kalıcı bir fiziksel görüntüye dönüştürdü. Niépce'in deneyi, görünen biçimlerin tamamen elle yeniden üretilmesi yerine doğrudan kaydedilebilmesini sağlayarak fotoğrafçılığın gelişiminde önemli bir erken adım oldu.`

};

window.historyTexts.tr.baktun.selecteren_levenspad = {

  title:
    "Serbest Bırakmak",

text:
  `Seçmek + Yaşam Yolu

<br><br>

1830'lar ve 1840'lar boyunca demiryolu ağları Avrupa'nın ve Kuzey Amerika'nın çeşitli bölgelerinde hızla genişledi. Raylar, kavşaklar ve makaslar, giderek daha fazla insanın ve yükün uzun mesafeler boyunca hareket edebildiği birbirine bağlı güzergâhlar oluşturdu.

<br><br>

Demiryolları, insanların nerede yaşayabilecekleri, çalışabilecekleri ve seyahat edebileceklerine ilişkin olanakları değiştirirken, kasabaları ve şehirleri giderek daha düzenli ulaşım ağlarıyla birbirine bağladı. Farklı hatlar ve varış noktaları arasında seçim yapmak, daha sonra dünya çapında genişlemeye devam edecek yeni bir hareket altyapısının parçası hâline geldi.`

};

window.historyTexts.tr.baktun.afstemmen_bewustzijn = {

  title:
    "Serbest Bırakmak",

text:
  `Uyumlamak + Bilinç

<br><br>

1879 yılında Wilhelm Wundt, Leipzig Üniversitesi'nde deneysel psikoloji için bir laboratuvar kurdu. Laboratuvar, kontrollü koşullar altında algı, dikkat, tepki süresi ve bilinçli deneyimin incelendiği önemli bir merkez hâline geldi.

<br><br>

Wundt'ın çalışmaları, psikolojinin bağımsız bir deneysel disiplin olarak yerleşmesine katkıda bulundu ve insan bilincinin ve zihinsel süreçlerin çeşitli yönlerini sistematik laboratuvar araştırmalarının konusu hâline getirdi.`

};

window.historyTexts.tr.baktun.transformeren_perspectief = {

  title:
    "Serbest Bırakmak",

text:
  `Dönüştürmek + Bakış Açısı

<br><br>

1905 yılında Albert Einstein özel görelilik teorisini yayımlayarak uzay ve zaman ölçümlerinin gözlemcilerin göreli hareketine bağlı olduğunu, buna karşılık boşluktaki ışık hızının eylemsiz gözlemciler için aynı kaldığını gösterdi.

<br><br>

Teori, uzay, zaman ve hareket arasındaki ilişkiyi dönüştürdü. Bir referans sisteminden eşzamanlı görünen olayların başka bir referans sisteminden eşzamanlı olması gerekmez; böylece evrensel zaman düşüncesinin yerini, ölçümlerin gözlemcinin hareket durumuyla ilişkili olduğu bir çerçeve aldı.`

};

window.historyTexts.tr.baktun.integreren_spoor = {

  title:
    "Serbest Bırakmak",

text:
  `Bütünleştirmek + İz

<br><br>

1929 yılında Edwin Hubble, galaksilerin uzaklıkları ile ölçülen uzaklaşma hızları arasında bir ilişki olduğunu gösteren gözlemler yayımladı. Daha uzaktaki galaksiler genel olarak daha büyük kırmızıya kaymalar göstererek kozmik ölçekte sistematik bir örüntüyü ortaya çıkardı.

<br><br>

Hubble, uzaklık tahminlerini spektroskopik ölçümlerle birleştirerek genişleyen bir evrene ilişkin gözlemsel kanıtların yerleşmesine katkıda bulundu. Bu ilişki, modern kozmolojinin ve evrenin büyük ölçekli davranışını ölçmenin temel araçlarından biri hâline geldi.`

};

window.historyTexts.tr.baktun.doorgronden_manifestatie = {

  title:
    "Serbest Bırakmak",

text:
  `Kavramak + Tezahür

<br><br>

1953 yılında James Watson ve Francis Crick, Rosalind Franklin, Maurice Wilkins ve çalışma arkadaşlarının önemli X-ışını kırınımı araştırmaları da dahil olmak üzere deneysel kanıtlardan yararlanarak DNA'nın çift sarmal yapısının bir modelini yayımladı.

<br><br>

Bu yapı, genetik bilginin fiziksel bir moleküler biçimde nasıl var olabileceğini ortaya koydu ve DNA'nın nasıl kopyalanabileceğine ilişkin bir mekanizma önerdi. Bu düzenin anlaşılması moleküler biyolojinin önemli temellerinden biri hâline geldi ve kalıtım ile canlı sistemlerin incelenmesini dönüştürdü.`

};

window.historyTexts.tr.baktun.ontsluiten_samenhang = {

  title:
    "Serbest Bırakmak",

text:
  `Açmak + Bütünlük

<br><br>

Tim Berners-Lee, 1989 ile 1991 yılları arasında CERN'de World Wide Web'i geliştirdi. Farklı bilgisayarlarda saklanan belgelerin hipermetin bağlantıları aracılığıyla birbirine bağlanabildiği ve ortak bir teknoloji kümesi kullanılarak erişilebildiği bir sistem oluşturdu.

<br><br>

Web; URL'leri, HTTP'yi, HTML'yi, web sunucularını ve tarayıcıları, yeni sayfalar ve bağlantılar eklendikçe genişleyebilen bütünlüklü bir bilgi sisteminde bir araya getirdi. İnterneti, öncelikle bilgisayarları birbirine bağlayan bir ağdan, giderek daha erişilebilir bir birbirine bağlı bilgi alanına dönüştürdü.`

};

window.historyTexts.tr.baktun.vrijmaken_essentie = {

  title:
    "Serbest Bırakmak",

text:
  `Serbest Bırakmak + Öz

<br><br>

2003 yılında İnsan Genom Projesi, insan genomunun neredeyse tamamını kapsayan yüksek kaliteli bir referans dizisinin tamamlandığını duyurdu. Uluslararası proje, milyarlarca DNA baz çiftini haritalandırıp dizilemiş ve elde edilen verileri dünyanın dört bir yanındaki araştırmacıların hızlı bir şekilde erişimine açmıştı.

<br><br>

Referans genom; insan genlerinin, kalıtsal çeşitliliğin ve biyolojik süreçlerin moleküler temelinin incelenmesi için ortak bir temel sağladı. Büyük ölçekli genomik bilgiyi daha ileri araştırmalar için geniş ölçüde erişilebilir hâle getirerek genetik ve biyomedikal araştırmaları dönüştürdü.`

};

window.historyTexts.tr.baktun.erkennen_waarheid = {

  title:
    "Dönüştürmek",

  text:
    `Kabul Etmek + Hakikat

<br><br>

MÖ 11 Eylül 3, İsa'nın olası doğum tarihi olarak önerilmiştir. Ernest L. Martin bu tarihi, Vahiy 12'de betimlenen imgelerin astronomik bir yorumuna dayanarak savundu; bu yorum daha sonra Kutsal Kitap araştırmacısı Michael S. Heiser tarafından da ele alındı ve desteklendi.

<br><br>

İsa, birinci yüzyılın Yahudi dini dünyasından ortaya çıktı ve Hristiyanlığın merkezi figürü haline geldi. Yaşamı ve öğretileri, daha sonra da ölümü ve dirilişi hakkındaki anlatılar, Roma dünyasına ve ötesine yayılmadan önce Yahudi bağlamı içinde gelişen bir hareketin temelini oluşturdu.`

};

window.historyTexts.tr.baktun.onderscheiden_samenhang = {

  title:
    "Dönüştürmek",

  text:
    `Ayırt Etmek + Bütünlük

<br><br>

Birinci yüzyılın ilk on yıllarında İsa ve onun ilk takipçileri Yahudi dini dünyasının bir parçasıydı. Öğretileri Yahudi gelenekleri, kutsal metinleri ve tartışmaları içinde gelişirken, çevresinde oluşan hareket zamanla kendine özgü ve tanınabilir bir karakter kazanmaya başladı.

<br><br>

Ölümünden sonra takipçileri, Yahudilik içinde kök salmış topluluklar olarak varlıklarını sürdürürken onun kimliği ve önemi hakkında kendilerine özgü inançlar geliştirdiler. Zamanla bu topluluklar başlangıçtaki çevrelerinin ötesine yayıldı ve Hristiyanlığın ayrı bir dini gelenek olarak kademeli biçimde ortaya çıkmasına katkıda bulundu.`

};

window.historyTexts.tr.baktun.plaatsen_essentie = {

  title:
    "Dönüştürmek",

  text:
    `Konumlandırmak + Öz

<br><br>

MS birinci yüzyılda İsa, Roma valisi Pontius Pilatus döneminde, büyük olasılıkla MS 30 civarında çarmıha gerildi. Ölümü, çevresinde oluşan hareketin sonunu getirmedi. Takipçileri onun ölümden dirildiğini ilan ettiler, bir araya gelmeye ve öğretilerini yaymaya devam ettiler.

<br><br>

Takip eden yıllarda İsa'nın kendisi gelişmekte olan hareketin merkezi referans noktası haline geldi. Kimliği, ölümü ve dirilişi hakkındaki inançlar hareketin mesajını giderek daha fazla tanımlarken, takipçi toplulukları oluşmaya ve Yahudiye'den Roma dünyasının diğer bölgelerine yayılmaya başladı.`

};